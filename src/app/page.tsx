...
export default function HomePage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-cyan-900">
        <div className="absolute inset-0 bg-gradient-to-b from-cyan-950/50 via-black to-black pointer-events-none" />
        <div className="mx-auto max-w-7xl px-4 py-24 sm:px-6 sm:py-32 lg:px-8 relative">
          <div className="text-center">
            <h1 className="text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl">
              حلول التيار الخفيف
              <br />
              <span className="text-cyan-400">والأنظمة الأمنية</span>
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-cyan-300/60 leading-relaxed">
            ...
            <div className="mt-10 flex items-center justify-center gap-4">
              <Link
                href="/products"
                className="inline-flex items-center gap-2 rounded-lg bg-cyan-600 px-6 py-3 text-sm font-medium text-white transition-all hover:bg-cyan-500"
              >
                تصفح المنتجات
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="mt-2 text-sm text-cyan-300/60">نقدم مجموعة متكاملة من الحلول الأمنية والذكية</p>
        </div>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((service) => (
            <div
              key={service.title}
              className="group rounded-xl border border-cyan-900/50 bg-cyan-950/30 p-6 text-center transition-all hover:border-cyan-700 hover:bg-cyan-900/50"
            >
              <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-cyan-900/50 group-hover:bg-cyan-800 transition-colors">
                <service.icon className="h-6 w-6 text-cyan-300" />
              </div>
              <h3 className="text-sm font-semibold text-white">{service.title}</h3>
              <p className="mt-2 text-xs text-cyan-300/60 leading-relaxed">{service.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* CTA */}
      <section className="border-t border-cyan-900">
